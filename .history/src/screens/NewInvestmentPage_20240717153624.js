    ...
    try {
      ...
      if (res?.status === 200) {
        const updateRes = await Transaction(
          isMounted,
          { OrderID: res.data, completed: true, refund: false },
          controller,
          auth,
          "patch"
        );
  ...
  return (
    <View style={styles.container}>
      <View style={styles.addNewCard}>
        ...
        <Text style={styles.enterYourCardNumberWrapper}>
          Harvest period: {item.duration}
        </Text>
        <Text style={styles.enterYourCardNumberWrapper}>
          Geo location: {item.geo_location}
        </Text>
        <Text style={styles.enterYourCardNumberWrapper}>
          Minimum invest: {item.minimum_invest}
        </Text>
        ...
        {investment.map((item, index) => {
          return (
            <View style={styles.addNewCard} key={index}>
              <Pressable
                onPress={() =>
                  navigation.navigate("NewInvestmentDetails", { item: item })
                }
              >
                <Text style={[styles.enterYourCardNumberWrapper]}>
                  Product: {item.product}
